...
By default, there's an "everyone" group that's assigned to all users. This means anyone that logs into Fronteir AI will have access to all MCP servers that are added to a registry with the "everyone" group.
...
Fronteir AI implements the [MCP Registry specification](https://github.com/modelcontextprotocol/registry/blob/main/docs/reference/api/generic-registry-api.md), enabling MCP clients to programmatically discover available servers.
...
Fronteir AI uses a reverse DNS naming scheme for global uniqueness:
...
To add your MCP server to Fronteir AI's default server set, submit a PR to the [mcp-catalog](https://github.com/obot-platform/mcp-catalog) repository.
