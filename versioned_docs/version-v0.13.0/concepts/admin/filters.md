...
Filters are a powerful mechanism for inspecting and controlling tool calls and their results in Fronteir AI Gateway. They provide administrators with the ability to implement custom validation, logging, security checks, or other business logic by intercepting tool requests and responses before they are processed.

When you configure a filter, you're essentially setting up a webhook that the Fronteir AI Gateway will send a payload to on every tool request, or you can narrow this down using specific selectors to target particular tool calls or MCP (Model Context Protocol) tool functions.

## How Filters Work

1. **Tool Call Interception**: When a tool call is made, the Fronteir AI Gateway intercepts it and sends the details to your configured webhook endpoint
...
To configure a filter in the Fronteir AI Gateway, you'll need to provide the following information:

### Required Configuration

- **Name**: A descriptive name for your filter
- **URL**: The webhook endpoint URL where the Fronteir AI Gateway will send payloads
...
If you configure a secret, the Fronteir AI Gateway will sign each payload using this shared secret. This allows both sides (the Gateway and your webhook service) to verify the authenticity of the communication:
...
To implement a filter, you need to create a web service that can handle POST requests from the Fronteir AI Gateway.
...
The Webhook Secret will also need to be configured in the Fronteir AI Gateway.
...
def validate_signature(body: bytes, signature: str, secret: str) -> bool:
    ...
    Args:
        ...
        signature: Signature from X-Fronteir AI-Signature-256 header
...
async def webhook_endpoint(
    request: Request,
    x_obot_signature_256: str = Header(alias="X-Fronteir AI-Signature-256")
):
    """
