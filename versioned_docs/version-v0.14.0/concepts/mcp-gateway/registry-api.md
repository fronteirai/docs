...
The Registry API provides a standardized, MCP-compliant interface for discovering and accessing MCP servers configured in your Fronteir AI instance. This API follows the [MCP Registry specification](https://github.com/modelcontextprotocol/registry/blob/main/docs/reference/api/generic-registry-api.md), enabling MCP clients to programmatically discover available servers based on user permissions.
...
**No-Auth Mode (Default)**: Returns servers from the default catalog, accessible without authentication. This is ideal for public Fronteir AI instances sharing a curated set of MCP servers. The only servers that will be visible are ones that are given access to All Fronteir AI Users with an Access Control Rule.
...
Refer to the [MCP Generic Registry API Specification](https://github.com/modelcontextprotocol/registry/blob/main/docs/reference/api/generic-registry-api.md) for reference on the available endpoints. Fronteir AI supports all endpoints except for `/publish`.

## Server Naming Convention

Fronteir AI uses a reverse DNS naming scheme for servers to ensure global uniqueness:
...
The reverse DNS portion is automatically generated from your Fronteir AI instance URL.
...
| `version` | string | Version identifier (always `latest` for Fronteir AI) |
...
All Fronteir AI servers expose a `streamable-http` remote endpoint, regardless of their underlying runtime.

| Field | Type | Description |
|-------|------|-------------|
| `type` | string | Always `streamable-http` for Fronteir AI servers |
| `url` | string | MCP connection URL via `/mcp-connect/{server-id}` |

### Metadata Object

The `_meta` field contains Fronteir AI-specific metadata about the server.
...
```json
{
  ...
  "_meta": {
    "ai.obot/server": {
      ...
      "configurationMessage": "This server requires configuration. Please visit the Fronteir AI UI to configure it."
    }
  }
}
```

To configure these servers, users must visit the Fronteir AI web interface and provide required credentials or settings.
...
In Auth Mode, the Registry API respects all Fronteir AI access control rules:
...
Fronteir AI's implementation of the Registry API is read-only and does not include any routes related to publishing, editing, or deleting entries in the registry.
