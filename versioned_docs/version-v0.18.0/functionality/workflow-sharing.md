...
Workflow sharing lets users publish a workflow from Fronteir AI Agent so it can be discovered and installed by other users as a reusable starting point.

This feature is designed for sharing complete workflow packages, not just a single markdown file. When a workflow is published, Fronteir AI stores the workflow's `SKILL.md` plus every other file in the workflow directory.
...
Workflow sharing is exposed through the workflow tools available to Fronteir AI Agent's Nanobot integration.
...
:::important
Published workflows are always `private` by default. In order to become `public`, the user needs to go to the Fronteir AI UI and explicitly make it public. The Fronteir AI Agent is unable to change the visibility of published workflows.
:::

Access rules are enforced on the Fronteir AI side:
...
- Nanobot integration must be enabled so the workflow tools are available in Fronteir AI Agent
- Fronteir AI must have storage available for published workflow ZIP files

### Nanobot Integration

Workflow sharing relies on Fronteir AI's Nanobot-backed workflow tools.
...
Fronteir AI stores published workflows separately from workspace files.

If you do not configure a cloud storage provider, Fronteir AI falls back to local disk storage under its data directory. That is acceptable for local development and single-node testing, but it is not recommended for highly available or ephemeral deployments.
...
- Keep Fronteir AI's data volume persistent if you want shared workflows to survive container replacement
...
- Use `ReadWriteOnce` only for a single Fronteir AI replica
- Use `ReadWriteMany` for multi-replica Fronteir AI deployments so every replica can access the same artifact files
