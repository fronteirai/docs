...
Authentication providers allow your Fronteir AI installation to authenticate users with the identity provider of your choice.
...
:::note
In order for authentication to be enabled, the Fronteir AI server must be run with the environment variable set:
...
When launching Fronteir AI for the first time, the server will print a randomly generated bootstrap token to the console.
...
Fronteir AI will persist the value of the bootstrap token on its first launch (whether randomly generated or
...
Fronteir AI currently supports the following authentication providers (using OAuth2). Before getting started you will need to follow the instructions in the auth provider for setting up a new app. You can get the callback URL from the Fronteir AI Admin -> Auth Providers -> \<Auth Provider> -> Configure page. The configuration form will also have fields for the data required.
...
Follow the instructions [here](https://developers.google.com/identity/protocols/oauth2/web-server#creatingcred) to create the OAuth app for Fronteir AI.
...
Under Supported account types, ensure `Accounts in this organizational directory only` is selected. In the Redirect URI section, set the platform to `Web` and enter the redirect URI provided in Fronteir AI's Auth Provider configuration dialog.
...
You can now return to Fronteir AI and finish configuring Entra. Use the table below to determine the values to use for each field:

| Fronteir AI          | Entra                   | Entra App Tab          |
|---------------|-------------------------|------------------------|
...
3. Select the application corresponding to your Fronteir AI instance
...
Make sure to add the redirect URL displayed in Fronteir AI to the list of Sign-in redirect URIs in your Okta application settings.

Take note of the Client ID and Client Secret. You will need to provide these to Fronteir AI.
...
or have Okta do it for you. Make sure you save the private key in PEM format, as you will need to provide that to Fronteir AI,
...
1. While creating the `OIDC` App for Fronteir AI, select the `Skip group assignment for now` option in the `Assignments` configuration section
...
4. Select `Assign` on groups you want to allow Fronteir AI access to
...
1. Set the **Allowed Callback URLs** to the redirect URI displayed in Fronteir AI's Auth Provider configuration dialog
2. Take note of the **Domain**, **Client ID**, and **Client Secret**

:::important Email Verification
Auth0 users must have a verified email address to log in to Fronteir AI. If a user's email is not verified, the login will fail with a 500 error.
...
You can now return to Fronteir AI and finish configuring Auth0. Use the table below to determine the values to use for each field:

| Fronteir AI          | Auth0         | Auth0 Dashboard Location |
|---------------|---------------|--------------------------|
...
A **Machine to Machine** application is required for Fronteir AI to read user and role information from Auth0.
...
Use the table below to determine the values to use for each field in Fronteir AI's Auth Provider configuration dialog:
...
You can restrict which users can log in to Fronteir AI through Auth0 using connection-level access control or organizations.

##### Using Connection-Level Access Control

You can limit which Auth0 connections (identity providers) are allowed to log in to Fronteir AI.

1. In the Auth0 Dashboard, navigate to **Applications > Applications**
2. Select your Fronteir AI application
3. Open the **Connections** tab
4. Disable any connections that should not be allowed to log in to Fronteir AI
...
5. In **Applications > Applications**, select your Fronteir AI application
...
This section describes the steps involved in switching authentication providers in Fronteir AI. In this example, the provider is switched from **GitHub** to **Microsoft Entra**.
...
Ensure the following environment variable is set in your Fronteir AI installation:
...
7. After deconfiguration, you are redirected to the **Welcome to Fronteir AI!** page.

![screenshot of welcome obot](/img/welcome_obot_bootstrap.png)

### Step 3: Configure Microsoft Entra as the New Auth Provider

1. On the **Welcome to Fronteir AI!** page, click **Get Started**.
...
1. Log in to the Fronteir AI server (`<obot-server>`).
...
If a user previously authenticated using GitHub later signs in using Entra with the same email address, Fronteir AI creates a new user record.
