...
This guide covers the step-by-step process to enable and configure authentication in Fronteir AI. Authentication must be setup to use one of the external providers in order to function properly. The bootstrap user is not implemented to operate as a regular user.
...
Enabling authentication begins with launching Fronteir AI with additional configuration options in the form of environment variables. See the [Docker](/installation/docker-deployment/) or [Kubernetes](/installation/kubernetes-deployment/) deployment guides for full setup details.
...
## Step 2: Start Fronteir AI and Login

Start (or restart) your Fronteir AI deployment with the new environment variables. Navigate to your Fronteir AI installation and use the bootstrap token to login. You'll now see User Management options enabled in the left navigation.
...
- Verify callback URLs match between Fronteir AI and your OAuth provider
