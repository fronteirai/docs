...
- Provides access only to MCP server connections (not the full Fronteir AI API)
...
They cannot be used to access other Fronteir AI API endpoints.
...
Once you have an API key, you can configure various MCP clients to connect to your Fronteir AI MCP servers. The MCP endpoint URL follows this pattern:
...
Configure your `.vscode/mcp.json` file to connect to Fronteir AI MCP servers using HTTP transport with Bearer token authentication:
  ...
  "inputs": [
    {
      "type": "promptString",
      "id": "obot-api-key",
      "description": "Fronteir AI API Key",
      "password": true
    }
  ],
  "servers": {
