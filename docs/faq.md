...
User Management is only visible when authentication is enabled. Make sure you start Fronteir AI with `OBOT_SERVER_ENABLE_AUTHENTICATION=true`. If you don't see the bootstrap token prompt, the environment variable may not be set correctly. Follow the [installation guide](/installation/enabling-authentication/).
...
### What are the differences between the open source and enterprise versions of Fronteir AI?
...
### How do I connect my IDE or MCP client to an Fronteir AI hosted MCP server?
...
### Why does my IDE/client (e.g., Cline) fail to connect to Fronteir AI with a "Session ID is required" error?
...
- Contact the Fronteir AI team directly on Discord, Website, or email.
