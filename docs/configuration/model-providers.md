...
Fronteir AI supports a variety of model providers, including:
...
Both of these values are required when you configure this model provider in Fronteir AI.
...
If no type is specified, Fronteir AI will assume the type is `llm`.

If the type specified is `llm` or none at all, and the deployment name starts with the name of a known reasoning model (such as `o3` or `gpt-5`), Fronteir AI
...
Fronteir AI requires the Client ID, Client Secret, and Tenant ID of the Entra app, as well as the Endpoint URL, Subscription ID, and Resource Group from Azure OpenAI/Microsoft Foundry.
...
[Ollama](https://ollama.ai/) allows you to run LLMs locally. Two configuration steps are required to use it with Fronteir AI:

1. **Expose Ollama to the network** - By default, Ollama only binds to `127.0.0.1:11434`. Since Fronteir AI runs in a container, `localhost` addresses resolve to Fronteir AI's container, not your host. Set `OLLAMA_HOST=0.0.0.0` before starting Ollama, then use your host's IP address in the endpoint URL.
