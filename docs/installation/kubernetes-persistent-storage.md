...
When Fronteir AI deploys MCP servers (including Fronteir AI Agent workloads) in Kubernetes, those pods need persistent volumes if you want workspace data to survive pod restarts and rescheduling.
...
## Configure Fronteir AI Agent Persistence
...
If `OBOT_ARTIFACT_STORAGE_PROVIDER` is unset, Fronteir AI stores published workflows on local disk at:
...
Mount it into Fronteir AI with:
...
This is the common setup when using a single Fronteir AI pod with a block-storage-backed `StorageClass`. It persists both general `/data` contents and the local published-artifact directory under `/data/.local/share/obot/published-artifacts`.

### Multiple Replicas with ReadWriteMany

For `replicaCount: 2` or higher, all Fronteir AI pods need concurrent access to the same `/data` volume. That requires a shared `ReadWriteMany` volume:
...
Use a `StorageClass` backed by a shared filesystem such as NFS. A single shared `ReadWriteOnce` claim is not suitable for multi-replica Fronteir AI deployments.
...
When enabled, the chart mounts that claim into the Fronteir AI container at `/data`, which includes the local published-artifact directory.
...
After installing the provisioner and creating its `StorageClass`, set that class in your Fronteir AI values file:
...
Then install or upgrade Fronteir AI:
