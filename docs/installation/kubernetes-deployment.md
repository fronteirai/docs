...
Deploy Fronteir AI on Kubernetes for production-grade reliability, scalability, and high availability.
...
- **HA Cluster**
- **CPU**: 4 cores for Fronteir AI
- **Memory**: 8GB for Fronteir AI

## Helm Installation

Fronteir AI provides a Helm chart for easy deployment [here](https://charts.obot.ai).
...
# Enable ingress or use a service of type loadbalancer to expose Fronteir AI
ingress:
  enabled: true
  hosts:
    - <your obot hostname>
...
- Restricts MCP servers to only communicate with Fronteir AI, DNS, and public internet
...
Fronteir AI applies Pod Security Standards to the MCP namespace using Pod Security Admission (PSA). The default configuration uses the **restricted** policy level for maximum security:
...
By default, Fronteir AI Agent uses storage inside its pod, which means all agent state is lost if the pod restarts. For production deployments, configure a persistent `StorageClass`.
...
If `OBOT_ARTIFACT_STORAGE_PROVIDER` is unset, Fronteir AI stores published workflows on local disk at `/data/.local/share/obot/published-artifacts`.
...
For a single Fronteir AI replica:
...
For multiple Fronteir AI replicas:
